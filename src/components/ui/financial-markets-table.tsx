...
export function FinancialTable({
    ...
    const isDark = theme === "dark" || !theme || theme === "system"; // graf is dark by default
    ...
    const formatLargeNumber = (amount: number, defaultUnit: string) => {
        if (defaultUnit === "B" && amount >= 1000) {
            return `${(amount / 1000).toFixed(1)}T`;
        }
        if (defaultUnit === "M" && amount >= 1000) {
            return `${(amount / 1000).toFixed(1)}B`;
        }
        return `${amount.toFixed(1)}${defaultUnit}`;
    };
            ...
            case "TW":
                return (
                    <svg width="32" height="32" viewBox="0 0 90 60" fill="none" className="scale-150">
                        ...
                        <rect width="45" height="30" fill="#000095" />
                        <circle cx="22.5" cy="15" r="7.5" fill="#FFFFFF" />
                    </svg>
                );
            default:
                return (
            ...
            <div className="bg-white border border-black/10 rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto text-black">
                    <div className="min-w-[1000px]">
                        {/* Table Headers */}
                        <div
                            className="px-8 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide bg-black/5 border-b border-black/10 text-left"
                            style={{
                                display: 'grid',
                        ...
                        >
                            ...
                            <div style={{ textAlign: 'left' }} className="pr-4">Daily perf.</div>
                        </div>
                                ...
                                <motion.div key={index.id} variants={rowVariants}>
                                    <div
                                        className={`px-8 py-3 cursor-pointer group relative transition-all duration-200 ${selectedIndex === index.id
                                            ? "bg-black/5 border-b border-graf-gold/50"
                                            : "hover:bg-black/5"
                                            } ${indexNum < indices.length - 1 && selectedIndex !== index.id ? "border-b border-black/5" : ""}`}
                                        style={{
                                            display: 'grid',
                                            gridTemplateColumns: '250px 100px minmax(60px, 1fr) minmax(60px, 1fr) minmax(60px, 1fr) minmax(60px, 1fr) minmax(80px, 1fr) minmax(60px, 1fr) minmax(100px, 1fr)',
                                            columnGap: '6px'
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="w-8 h-8 rounded-full overflow-hidden border border-black/10 flex items-center justify-center bg-white shadow-sm">
                                                <div className="w-full h-full">
                                                    {getCountryFlag(index.countryCode)}
                                                </div>
                                            </div>
                                            <div className="min-w-0">
                                                <div className="font-medium text-black truncate">{index.name}</div>
                                                <div className="text-xs text-gray-500">{index.country}</div>
                                            </div>
                                        </div>
                                        ...
                                        {/* Dividend Yield */}
                                        <div className="flex items-center">
                                            <span className="font-semibold text-graf-gold">
